...
X_train, X_test, y_train, y_test = train_test_split(input_data, output_data, test_size=0.65, random_state=42)
...
output_path = 'mappings/mapped_numbers_train.csv'
...
filename = "counts/binary_representation_counts_train.json"
...
shared_dense1 = Dense(1024, activation='elu')
shared_dense2 = Dense(512, activation='elu')
...
df_predictions.to_csv('predictions/predictions_test_1H_0.5_10-5_1_800.csv', index=False)

# Write to Excel
with pd.ExcelWriter('training_details/training_details_1H_0.5_10-5_1_800.xlsx', engine='openpyxl') as writer:
...
def predict_single_binary_value(binary_array, model):
    ...
    prediction = model.predict([binary_array] * 2)  
    
    # Return the prediction divided by 2
...
with open('predictions/predicted_individuals_1H_0.5_10-5_1_800.json', 'w') as json_file:
