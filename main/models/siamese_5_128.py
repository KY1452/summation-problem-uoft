...
input_data = np.load('numpy_arrays/input_data_6_modified.npy')
...
# Save to CSV
df_X_train.to_csv('train_eval/df_X_train_128.csv', index=False)
df_y_train.to_csv('train_eval/df_y_train_data_128.csv', index=False)
df_X_test.to_csv('train_eval/df_X_test_128.csv', index=False)
df_y_test.to_csv('train_eval/df_y_test_data_128.csv', index=False)
...
output_path = 'mappings/mapped_numbers_train_128.csv'
...
# Save the DataFrame to a CSV
output_path = 'mappings/mapped_numbers_test_128.csv'
...
filename = "counts/binary_representation_counts_train_128.json"
...
filename_val = "counts/binary_representation_counts_test_128.json"
...
shared_dense1 = Dense(1024, activation='linear')
shared_dense2 = Dense(512, activation='linear')
...
df_predictions.to_csv('predictions/predictions_test_128.csv', index=False)

# Write to Excel
with pd.ExcelWriter('training_details/training_details_128.xlsx', engine='openpyxl') as writer:
...
# Generate all 128 unique one-hot binary representations
...
with open('predictions/predicted_individuals_128.json', 'w') as json_file:
