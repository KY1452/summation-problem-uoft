...
input_data = np.load('numpy_arrays/input_data_5.npy')
output_data = np.load('numpy_arrays/output_data_5.npy')
...
for _, row in df_X_train.iterrows():
    numbers = []
    for i in range(10): #change for number of atoms
...
X_train_numbers = [X_train[:, j * binary_digits: (j + 1) * binary_digits] for j in range(10)] #change for number of atoms
X_test_numbers = [X_test[:, j * binary_digits: (j + 1) * binary_digits] for j in range(10)] #change for number of atoms
...
input_numbers = [Input(shape=(binary_digits,), name=f'input_number_{i}') for i in range(10)] #change for number of atoms
...
def predict_single_binary_value(binary_array, model):
    ...
    prediction = model.predict([binary_array] * 10)  #change for number of atoms
...
for binary_str in binary_representations:
    ...
    prediction = model.predict([binary_array] * 10)   #change for number of atoms
    predicted_results[binary_str] = float(prediction[0][0] / 10) #change for number of atoms
