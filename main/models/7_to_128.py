...
X = np.array([list(map(int, list(key))) for key in data_7_bits.keys()])
print(X)
y = np.array([list(map(int, list(key))) for key in data_128_bits.keys()])
print(y)
...
#     weight = tf.where(tf.equal(y_true, 1), 10**4.0, 1.0)
...
output_layer = Dense(128, activation='relu')(h2)
...
predictions.to_excel('predictions/test_7_to_128.xlsx', index=False)

predicted_classes = np.argmax(predicted, axis=1)
actual_classes = np.argmax(y_test, axis=1)
class_comparison_df = pd.DataFrame({'Predicted Class': predicted_classes, 'Actual Class': actual_classes})
class_comparison_df.to_csv('predictions/class_predictions_vs_actual_7_to_128.csv', index=False)


plot_model(model, to_file='model_plot_7_to_128.png', show_shapes=True, show_layer_names=True)
