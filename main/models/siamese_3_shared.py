...
# Shared Network for all input numbers
...
# Pass each binary input through the shared layers
...
history = model.fit(X_train_numbers, y_train, validation_data=(X_val_numbers, y_val), epochs=60, batch_size=batch_size, verbose=1)
...
df_loss = pd.DataFrame({
    'Epoch': list(range(1, 61)),  # Assuming you're training for 300 epochs
...
def predict_single_binary_value(binary_str, model):
    ...
    prediction = model.predict([binary_array] * 3)  # Now, we're replicating the single binary input for all three shared subnetworks
...
with open('predicted_results_S3_shared.json', 'w') as json_file:
