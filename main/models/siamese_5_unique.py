...
def efficient_random_combination_sample(iterable, repeat, sample_size):
    ...
    while len(sampled_combinations) < sample_size:
        ...
        # Convert the combination to a tuple for hashability
...
# Save to CSV
df_X_train.to_csv('X_train_data.csv', index=False)
df_y_train.to_csv('y_train_data.csv', index=False)
df_X_test.to_csv('X_test_data.csv', index=False)
df_y_test.to_csv('y_test_data.csv', index=False)

X_train_data = pd.read_csv('X_train_data.csv', header=None)
X_test_data = pd.read_csv('X_test_data.csv', header=None)
...
for _, row in X_train_data.iterrows():
    numbers = []
    for i in range(5):
...
mapped_numbers_df_train.to_csv(output_path, index=False, header=True)

for _, row in X_test_data.iterrows():
    numbers = []
    for i in range(5):
        # Extract the 128-bit fragment
        fragment = ''.join(map(str, row[i*128:(i+1)*128].tolist()))
...
mapped_numbers_df_test.to_csv(output_path, index=False, header=True)
...
# history = model.fit(X_train_numbers, y_train, validation_data=(X_test_numbers, y_test), epochs=epochs, batch_size=batch_size, verbose=1)
