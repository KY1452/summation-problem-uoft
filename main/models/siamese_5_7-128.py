...
input_data = np.load('numpy_arrays/input_data_60.npy')
output_data = np.load('numpy_arrays/output_data_60.npy')

X_train, X_test, y_train, y_test = train_test_split(input_data, output_data, test_size=0.90, random_state=42)
...
# Save to CSV
df_X_train.to_csv('train_eval/df_X_train_7i.csv', index=False)
df_y_train.to_csv('train_eval/df_y_train_data_7i.csv', index=False)
df_X_test.to_csv('train_eval/df_X_test_7i.csv', index=False)
df_y_test.to_csv('train_eval/df_y_test_data_7i.csv', index=False)
...
for _, row in df_X_train.iterrows():
    numbers = []
    for i in range(60): #change for number of atoms
...
output_path = 'mappings/mapped_numbers_train_7i.csv'
...
# Save the DataFrame to a CSV
output_path = 'mappings/mapped_numbers_test_7i.csv'
mapped_numbers_df_test.to_csv(output_path, index=False, header=False)

X_train_numbers = [X_train[:, j * binary_digits: (j + 1) * binary_digits] for j in range(60)] #change for number of atoms
X_test_numbers = [X_test[:, j * binary_digits: (j + 1) * binary_digits] for j in range(60)] #change for number of atoms
...
filename = "counts/binary_representation_counts_train_7i.json"
...
filename_val = "counts/binary_representation_counts_test_7i.json"
with open(filename_val, 'w') as json_file:
    json.dump(binary_representation_counts_test, json_file, indent=4)

input_numbers = [Input(shape=(binary_digits,), name=f'input_number_{i}') for i in range(60)] #change for number of atoms
...
shared_encoder_dense1 = Dense(128, activation='relu', name='shared_encoder_dense1')
# shared_encoder_dense2 = Dense(128, activation='relu', name='shared_encoder_dense2')

# Define shared decoder layers
shared_decoder_dense1 = Dense(1024, activation='relu', name='shared_decoder_dense1')
shared_decoder_dense2 = Dense(512, activation='relu', name='shared_decoder_dense2')

def encoder(input_tensor):
    x = shared_encoder_dense1(input_tensor)
    encoded_output = x
    # encoded_output = shared_encoder_dense2(x)
    return encoded_output

def decoder(input_tensor):
    x = shared_decoder_dense1(input_tensor)
    decoded_output = shared_decoder_dense2(x)
    return decoded_output

def build_autoencoder():
    input_numbers = [Input(shape=(7,), name=f'input_number_{i}') for i in range(60)]  # Assuming 5 inputs

    encoded_outputs = [encoder(inp) for inp in input_numbers]
    decoded_outputs = [decoder(encoded) for encoded in encoded_outputs]

    combined = Add()(decoded_outputs)
...
df_predictions.to_csv('predictions/predictions_test_7i.csv', index=False)

# Write to Excel
with pd.ExcelWriter('training_details/training_details_7i.xlsx', engine='openpyxl') as writer:
...
plt.tight_layout()
plt.savefig(os.path.join(plot_folder, 'trainvallossi.png'))
plt.show()
...
def identify_and_plot_outliers_v2(actual_values, predicted_values, std_multiplier=3):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(plot_folder, 'sum_outliersi.png'))
    plt.show()

identify_and_plot_outliers_v2(y_test, test_predictions)
# identify_and_plot_outliers_v2(y_train, test_predictions)

encoder_output = model.get_layer('shared_encoder_dense1').output
...
for binary_str in binary_representations:
    ...
    encoded_bits_128 = encoder_model.predict([binary_array]*60)

    result_dict = {'Binary 7-Bit': binary_str}
    result_dict.update({f'Bit{i+1}': encoded_bits_128.flatten()[i] for i in range(128)})
    predicted_results.append(result_dict)

df = pd.DataFrame(predicted_results)
excel_file_path = 'predictions/predicted_individuals_7-128i.xlsx'
df.to_excel(excel_file_path, index=False)

predicted_results = {}
for binary_str in binary_representations:
    binary_array = np.array(list(map(int, list(binary_str)))).reshape(1, 7)
    prediction = model.predict([binary_array] * 60)   #change for number of atoms
    predicted_results[binary_str] = float(prediction[0][0] / 60) #change for number of atoms

with open('predictions/predicted_individuals_7i.json', 'w') as json_file:
...
def identify_and_plot_outliers_v1(actual_values, predicted_values, std_multiplier=3):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(plot_folder, 'individual_outliersi.png'))
    plt.show()
...
plot_model(model, to_file='model_plot_7-128.png', show_shapes=True, show_layer_names=True)