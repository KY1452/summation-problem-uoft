...
X_train_numbers = [X_train[:, j * binary_digits: (j + 1) * binary_digits].reshape((-1, 1, binary_digits)) for j in range(5)]
X_test_numbers = [X_test[:, j * binary_digits: (j + 1) * binary_digits].reshape((-1, 1, binary_digits)) for j in range(5)]
...
input_numbers = [Input(shape=(1, binary_digits), name=f'input_number_{i}') for i in range(5)]

shared_dense1 = LSTM(256, return_sequences=True)
shared_dense2 = LSTM(128, return_sequences=False)
shared_dense3 = Dense(64, activation='relu')
...
def predict_single_binary_value(binary_array, model):
    reshaped_input = binary_array.reshape(1, 1, 7)
    prediction = model.predict([reshaped_input] * 5)
...
for binary_str in binary_representations:
    binary_array = np.array(list(map(int, list(binary_str)))).reshape(1, 1, 7) 
    prediction = model.predict([binary_array] * 5)  
    predicted_results[binary_str] = float(prediction[0][0]/5)
...
plot_model(model, to_file='model_plot_lstm_7.png', show_shapes=True, show_layer_names=True)