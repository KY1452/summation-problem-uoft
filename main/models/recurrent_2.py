...
# input_data = np.load('numpy_arrays/input_data_2.npy')
# output_data = np.load('numpy_arrays/output_data_2.npy')
...
# # Save to CSV
# df_X_train.to_csv('train_eval/df_X_train_copy.csv', index=False)
# df_y_train.to_csv('train_eval/df_y_train_data.csv', index=False)
# df_X_test.to_csv('train_eval/df_X_test.csv', index=False)
# df_y_test.to_csv('train_eval/df_y_test_data.csv', index=False)

# Get from shuffled CSV
df_X_train = pd.read_csv('train_eval/df_X_train copy.csv')
...
output_path = 'mappings/mapped_numbers_train_copy.csv'
...
# Save the DataFrame to a CSV
output_path = 'mappings/mapped_numbers_test.csv'
mapped_numbers_df_test.to_csv(output_path, index=False, header=False)

X_train_numbers = [X_train[:, j * binary_digits: (j + 1) * binary_digits].reshape((-1, 1, binary_digits)) for j in range(2)]
X_test_numbers = [X_test[:, j * binary_digits: (j + 1) * binary_digits].reshape((-1, 1, binary_digits)) for j in range(2)]

individual_binary_representations_train = [row for sublist in X_train_numbers for row in sublist]
binary_representations_train_individual = [''.join(map(str, row.flatten())) for row in individual_binary_representations_train]
...
filename = "counts/binary_representation_counts_train_copy.json"
...
binary_representations_test_individual = [''.join(map(str, row.flatten())) for row in individual_binary_representations_test]
...
filename_val = "counts/binary_representation_counts_test.json"
with open(filename_val, 'w') as json_file:
    json.dump(binary_representation_counts_test, json_file, indent=4)

input_numbers = [Input(shape=(1, binary_digits), name=f'input_number_{i}') for i in range(2)]

def shared_subnetwork(input_tensor):
    print(input_tensor.shape)
    x = LSTM(256, return_sequences=False)(input_tensor) 
    # h1 = LSTM(128, return_sequences=False)(x) 
    h2 = Dense(64, activation='linear')(x)
    return h2
...
df_predictions.to_csv('predictions/predictions_test_1H_0.5_10-5LRS_1_800_copy.csv', index=False)

# Write to Excel
with pd.ExcelWriter('training_details/training_details_1H_0.5_10-5LRS_1_800_copy.xlsx', engine='openpyxl') as writer:
...
def predict_single_binary_value(binary_array, model):
    reshaped_input = binary_array.reshape(1, 1, 128)

    prediction = model.predict([reshaped_input] * 2)
...
for i in range(128):
    ...
    predicted_results[binary_str] = float(predict_single_binary_value(binary_array, model))

with open('predictions/predicted_individuals_1H_0.5_10-5LRS_1_800_copy.json', 'w') as json_file:
...
def identify_and_plot_outliers_v1(actual_values, predicted_values, std_multiplier=3):
    ...
    plt.title("Scatter plot between Actual and Predicted Individual Values with Outliers Highlighted")
