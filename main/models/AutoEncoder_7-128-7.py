...
def load_and_prepare_data(path_128_bits, path_7_bits):
    ...
    X = np.array([binary_string_to_numpy_array(key) for key in data_7_bits.keys()])
    y = np.array([binary_string_to_numpy_array(key) for key in data_128_bits.keys()])
...
encoded1 = Dense(32, activation='relu')(input_layer)    # Encoder
encoded2 = Dense(64, activation='relu')(encoded1)
latent_representation = Dense(128, activation='softmax')(encoded2)

decoded1 = Dense(128, activation='relu')(latent_representation)    # Decoder
decoded2 = Dense(64, activation='relu')(decoded1)
decoded_output = Dense(7, activation='relu')(decoded2)
...
def save_128_bit_comparisons(encoder, x_data, y_actual, file_name):
    predicted_128_bit = encoder.predict(x_data)
    print(predicted_128_bit.shape)  # This should be (n_samples, 7) based on your description
    print(y_actual.shape)         # Also expected to be (n_samples, 7)
    df = pd.DataFrame(np.hstack((predicted_128_bit, y_actual)), 
...
def save_7_bit_comparisons(autoencoder, x_data, file_name):
    predicted_7_bit = autoencoder.predict(x_data)
    df = pd.DataFrame(np.hstack((predicted_7_bit, x_data)), 
...
save_128_bit_comparisons(encoder, X_test, y_test, '128_bit_comparisons_7-128-7.xlsx')
save_7_bit_comparisons(autoencoder, X_test, '7_bit_comparisons_7-128-7.xlsx')

plot_model(autoencoder, to_file='model_plot_7-128-7.png', show_shapes=True, show_layer_names=True)
